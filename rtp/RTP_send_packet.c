/* * 
 *  $Id: RTP_send_packet.c 351 2006-06-01 17:58:07Z shawill $
 *  
...
int RTP_send_packet(RTP_session *session)
{
    ...
    RTP_header r;      /*12 Bit的RTP头*/
    int res=ERR_GENERIC;
    double s_time;
    double nextts;           /*下一个时间戳*/
    OMSSlot *slot=NULL;
    ...
    if(!(slot = OMSbuff_getreader(session->cons)))
    {
        ...
        {
            DEBUG_PRINT_ERROR("get frame error!")
            sprintf(szDebug,"get_frame return:%d \n ",res);
    ...
    /*This runs if the consumer reads slot written in another RTP session*/
    ...
    while (slot) 
    {
        ...
        r.timestamp=htonl(session->start_rtptime+msec2tick(slot->timestamp, session->current_media)-session->cons->firstts);	
        ...
        if ((psize_sent=RTP_sendto(session, rtp_proto, packet, slot->data_size + hdr_size))<0)
        {
        ...
        if ( (nextts == -1) || (nextts != s_time) )
        {
            ...
            session->current_media->mtime += session->current_media->description.pkt_len;
            slot = NULL;
            session->cons->frames--;
        } else
            slot = OMSbuff_getreader(session->cons);
