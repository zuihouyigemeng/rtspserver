/* * 
 *  $Id: RTCP_handler.c 348 2006-05-30 16:41:33Z shawill $
 *  
...
int RTCP_handler(RTP_session *session)
{
    ...
    if (session->rtcp_stats[i_server].pkt_count%20==0) 
    {
        ...
        {
            if (RTP_sendto(session, rtcp_proto, session->rtcp_outbuffer, session->rtcp_outsize)<0)
                fnc_log(FNC_LOG_VERBOSE,"RTCP Packet Lost\n");
            session->rtcp_outsize=0;                         /*发送之后清空缓冲区数据*/
