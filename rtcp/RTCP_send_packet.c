        ...
        case SR: 
            {
                ...
                hdr.length=htons((pkt_size >> 2) -1);    /*用32位字表示的数减去1*/
                ...
                hdr_sr.rtp_timestamp = htonl((unsigned int)((double)ntp_time.tv_sec 
                                                        + (double)ntp_time.tv_usec/1000000.) * session->current_media->description.clock_rate 
