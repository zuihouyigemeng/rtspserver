/* * 
 *  $Id: read_MP3.c 133 2005-05-09 17:35:14Z federico $
 *  
...
int read_MP3(media_entry *me,uint8 *data,uint32 *data_size,double *mtime, int *recallme, uint8 *marker)
{
    ...
    *recallme=0;
    ...
    if(me->play_offset!=-1/*me->prev_mstart_offset*/ && (me->description).msource!=live)
    {                
        //random access 
        N=(float)me->description.bitrate/8 * me->play_offset/1000;
        /*me->prev_mstart_offset=*/
        me->play_offset=-1;// pkt_len is pkt lenght in milliseconds,
        lseek(me->fd,N,SEEK_SET);
    }

    if ((read(me->fd,&(buff[me->buff_size]),4)) != 4 )
    {
        DEBUG_PRINTF("<#><#><#><#>end of file occured!return \n");
        return ERR_EOF;
    }
