...
int validate_stream(media_entry *p, SD_descr ** sd_descr)
{
    ...
    {
        DEBUG_PRINT_INFO("cannot find filename!")
        return ERR_PARSE;
    }       
    if (!(p->description.flags & MED_PAYLOAD_TYPE)) 
    {
        DEBUG_PRINT_INFO("cannot find payload type")
        return ERR_PARSE;
    }       
    if (!(p->description.flags & MED_PRIORITY)) 
    {
        DEBUG_PRINT_INFO("cannot find priority!")
        return ERR_PARSE;
    }       
    ...
    * */
    ...
    res=register_media(p);                /*register media*/
    
    if(res==ERR_NOERROR)
    {
        return p->media_handler->load_media(p);                     /*load media*/
    }
