/* * 
 *  $Id: parse_SD_file.c 247 2005-11-22 08:54:09Z shawill $
 *  
...
/*read and parse sd files,fill in sd_descr*/
int parse_SD_file(char *object,SD_descr *sd_descr)
{
    ...
    sprintf(szDebug,"file name is:%s",thefile);
    ...
    do {
            ...
            while (strcasecmp(keyword,SD_STREAM_END)!=0 && !feof(f)) 
            {
                ...
                /********END CC*********/
                /*just for debug*/
                sprintf(szDebug,"media flags is :%d",p->description.flags);
                DEBUG_PRINT_INFO(szDebug)            
                
            }
            if ((res = validate_stream(p,&sd_descr)) != ERR_NOERROR) {
                    sprintf(szDebug,"validate stream error!return code:%d",res);
                    DEBUG_PRINT_INFO(szDebug)
                    return res;
        }
    } while (!feof(f));     
