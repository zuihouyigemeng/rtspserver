...
uint32 mediacpy(media_entry **media_out, media_entry **media_in) 
{
    ...
    {     
      if (!strcasecmp((*media_out)->description.encoding_name, "RTP_SHM")) 
        {
            if ( !((*media_out)->pkt_buffer = OMSbuff_shm_map((*media_out)->filename)) )
                return ERR_ALLOC;
        } 
        else  
        {
            ...
            {
                DEBUG_PRINTF("malloc media_out->pkt_buffer,calling omsbuffer module error!");
                return ERR_ALLOC;
            }
        }
    }
