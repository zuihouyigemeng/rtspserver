/* * 
 *  $Id: load_MPA.c 284 2006-02-01 11:49:23Z shawill $
 *  
...
int load_MPA(media_entry *p)
{
    ...
    if (!memcmp(p->buff_data, "ID3", 3)) 
    { 
        /*ID3 tag present*/
        n = read_dim(p->fd, &tag_dim); // shawill: one day we will look also at this function

#if 0
        /*just for debug, commented by yanf,Sep15,2006
        * if it be a malformed mp3, ID3 tag cannot be processed successfully,so  we have to mannually 
        * change the position of the file pointer to read valid data,here the original function to change file positon does't work
        */
        tag_dim = 0x1100;
        sprintf(szDebug,"media dim is %d\n",tag_dim);
        DEBUG_PRINT_INFO(szDebug)   
        sprintf(szDebug,"read_dim return :%d",n);
        DEBUG_PRINT_INFO(szDebug)
        lseek(p->fd,tag_dim,SEEK_SET);   /*SEEK TO ABSOLUTE ,TEST,yanf*/
#endif  
        ...
        if ( (p->buff_size = read(p->fd, p->buff_data, 4)) != 4) 
        {
            DEBUG_PRINT_ERROR("error read in load_MPA!")
            return ERR_PARSE;
        }
    ...
    #if 0       /*just for test a malformed mp3 file,yanf*/
    sprintf(szDebug,"the data is:%x-%x",p->buff_data[0],p->buff_data[1]);
    DEBUG_PRINT_INFO(szDebug);
    #endif
    ...
    {
        /*just for test ,yanf*/
        DEBUG_PRINT_ERROR("error data in load_MPA")
        sprintf(szDebug,"buffer data is %x",p->buff_data);
        DEBUG_PRINT_INFO(szDebug)
    ...
    switch (p->buff_data[1] & 0x1e)
    {                
        /* Mpeg version and Level */
        ...
        default: 
            {
                DEBUG_PRINT_ERROR("unsurpported mpeg level")
                return ERR_PARSE;
            }
    }
    ...
    {     /* Mpeg-1 */
        switch (p->buff_data[2] & 0x0c) 
        {
            ...
            default: 
            {
                DEBUG_PRINT_ERROR("unsurpported sample rate in mpeg1!")
                return ERR_PARSE;
            }
        }
    }
    ...
    {               
        ...
        default: 
            {
                DEBUG_PRINT_ERROR("unsurpported sample rate in mpeg2!")
                return ERR_PARSE;
            }
        }
    }
