...
int get_SDP_descr(media_entry *media,char *descr,int extended,char *url)
{
    ...
    {
        sprintf(szDebug," enum result code is : %d ",res);
    ...
    /*DEBUG_PRINTF(szDebug); */                 /*just for debug*/
    ...
    if (p==NULL)
    {
        DEBUG_PRINT_ERROR("error searching media!")
        return ERR_PARSE;
     }
...
#if 0      /*yanf for debug*/
    DEBUG_PRINTF("-------------------------descr----------------------------------------\n");
    DEBUG_PRINTF(descr);DEBUG_PRINTF("\n");
    DEBUG_PRINTF("-------------------------end of descr----------------------------------------\n");    
#endif                     
    return ERR_NOERROR;
}
