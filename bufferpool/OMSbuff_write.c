/* * 
 *  $Id: OMSbuff_write.c 333 2006-03-31 18:40:05Z shawill $
 *  
 ...
 *  */
int32 OMSbuff_write(OMSBuffer *buffer, uint64 seq, uint32 timestamp, uint8 marker, uint8 *data, uint32 data_size)
{
    OMSSlot *slot = &buffer->slots[buffer->control->write_pos];      /*上一次写位置指针*/
    uint64 curr_seq = slot->slot_seq;
    OMSSlot *valid_read_pos = &buffer->slots[buffer->control->valid_read_pos];  /*上一次合法读位置指针*/
    double ts;
    ...
    {
        if (buffer->slots[slot->next].refs > 0)      /*已经被引用，需要创建新的页*/
        {
        ...
        {
            slot = &buffer->slots[slot->next];  /*当前写位置*/
            // write_pos reaches valid_read_pos, we "push" it
            if ( (valid_read_pos->slot_seq) && (valid_read_pos == slot) ) 
            {
                for (ts=valid_read_pos->timestamp;\
                            (buffer->slots[valid_read_pos->next].slot_seq) && (ts==valid_read_pos->timestamp);\
                            ts=valid_read_pos->timestamp, valid_read_pos=&buffer->slots[valid_read_pos->next]);
    ...
    buffer->control->write_pos = OMStoSlotPtr(buffer, slot);   /*标记写位置*/
