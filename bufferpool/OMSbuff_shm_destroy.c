/* * 
 *  $Id: OMSbuff_shm_destroy.c 292 2006-02-13 17:36:12Z shawill $
 *  
...
int OMSbuff_shm_destroy(OMSBuffer *buffer)
{
    ...
    if ( !(shm_file_name = fnc_ipc_name(buffer->filename, OMSBUFF_SHM_CTRLNAME)) )
        return 1;
    if ( shm_unlink(shm_file_name) ) 
    {
        ...
        case EACCES:
            fnc_log(FNC_LOG_ERR, "Permission denied on SHM Object %s\n");
            break;
            
        default:
            fnc_log(FNC_LOG_ERR, "Could not unlink SHM Object %s\n");
            break;
        }
