...
/*** control struct 的共享内存映射 ***/
    ...
    if ( (fd < 0) ) 
    {
        fnc_log(FNC_LOG_ERR, "Could not open POSIX shared memory (OMSControl): is Felix running?\n");
        return NULL;
    }
    ...
    {
        fnc_log(FNC_LOG_ERR, "Strange size for OMSControl shared memory! (not an integer number of slots)\n");
        close(fd);
        return NULL;
    }
    
    control = mmap(NULL, fdstat.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);    /*映射*/
    close(fd);
    ...
    {
        fnc_log(FNC_LOG_ERR, "Strange size for shared memory! (not an integer number of slots)\n");
        close(fd);
        munmap(control, sizeof(OMSControl));
        return NULL;
    }
    
    slots = mmap(NULL, fdstat.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);/*映射*/
    close(fd);
