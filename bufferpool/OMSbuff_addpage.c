/* * 
 *  $Id: OMSbuff_addpage.c 327 2006-03-28 16:14:41Z shawill $
 *  
...
OMSSlot *OMSbuff_addpage(OMSBuffer *buffer, OMSSlot *prev)
{
    ...
    switch (buffer->type) 
    {
        case buff_shm:
            ...
            fnc_log(FNC_LOG_DEBUG,"OMSSlots page added in SHM memory (%u slots)\n", buffer->known_slots);
            break;
            
        case buff_local:
            prev_diff = prev - buffer->slots;
            if ( !(added=realloc(buffer->slots, (buffer->control->nslots+OMSBUFF_MEM_PAGE)*sizeof(OMSSlot))) )
            return NULL;
            buffer->slots = added;

            prev = buffer->slots + prev_diff;
            // init new slots
            for (i=buffer->control->nslots; i<buffer->control->nslots+OMSBUFF_MEM_PAGE-1; i++) {
            added[i].refs = 0;
            added[i].slot_seq = 0;
            added[i].next = i + 1;
            }
            ...
            fnc_log(FNC_LOG_DEBUG,"OMSSlots page added in local bufferpool (%u slots)\n", buffer->known_slots);
            break;
