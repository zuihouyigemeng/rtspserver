...
#define LR 2 // last_read_pos
    ...
    {
        pointers[WR] = (buffer->control->write_pos == (OMSSlotPtr)i) ? 'W' : ' ';
        pointers[RD] = (cons && (cons->read_pos == (OMSSlotPtr)i)) ? 'R' : ' ';
        pointers[LR] = (cons && (cons->last_read_pos == (OMSSlotPtr)i)) ? 'L' : ' ';
        fprintf(dump_file, "refs: %u\tseq: %llu   \ttime: %3.2f\tslot: %u   \tnext: %d  \t%s\n", buffer->slots[i].refs, 
        buffer->slots[i].slot_seq, buffer->slots[i].timestamp, i, buffer->slots[i].next, pointers);
    }
    fprintf(dump_file, "--------- BUFFERPOOL DUMP ---------\n");
    }
