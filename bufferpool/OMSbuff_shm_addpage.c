/* * 
 *  $Id: OMSbuff_shm_addpage.c 326 2006-03-24 17:27:46Z shawill $
 *  
...
/* This function adds shared memory page of slots.
...
OMSSlot *OMSbuff_shm_addpage(OMSBuffer *buffer)
{
    ...
    //char *shm_file_name;    /*yanf*/
    int fd;
    struct stat fdstat;

#if 0
     /*** slots共享内存映射***/
    if ( !(shm_file_name = fnc_ipc_name(buffer->filename, OMSBUFF_SHM_SLOTSNAME)) ) return NULL;

    fd = shm_open(shm_file_name, O_RDWR, 0);
    free(shm_file_name);
#endif
    fd = open("../shm",O_RDWR,0);
    /*检查可能的错误*/
    if ( (fd < 0) )
    {
        fnc_log(FNC_LOG_ERR, "Could not open POSIX shared memory (OMSSlots): is Felix running?\n");
        return NULL;
    }
    ...
    if ( ((size_t)fdstat.st_size != buffer->control->nslots*sizeof(OMSSlot)) ) 
    {
        fnc_log(FNC_LOG_ERR, "Strange size for shared memory! (not the number of slots reported in control struct)\n");
        close(fd);
        return NULL;
    }
    ...
    /*映射变量*/
    slots = mmap(NULL, (buffer->control->nslots + OMSBUFF_SHM_PAGE) * sizeof(OMSSlot), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    close(fd);
    ...
    for (i=buffer->control->nslots; i<buffer->control->nslots+OMSBUFF_SHM_PAGE-1; i++) 
    {
        slots[i].refs = 0;
        slots[i].slot_seq = 0;
        slots[i].next = i + 1;
    ...
    slots[i].refs = 0;
    slots[i].slot_seq = 0;
     /*last added slot in shm new page has next slot to NULL: OMSbuff_slotadd will link it correctly.*/ 
     slots[i].next = -1;
