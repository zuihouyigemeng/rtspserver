/* * 
 *  $Id: OMSbuff_getslot.c 333 2006-03-31 18:40:05Z shawill $
 *  
...
OMSSlot *OMSbuff_getslot(OMSBuffer *buffer)
{
    ...
    {
        DEBUG_PRINTF("######calling shm_refresh in omsbuff_getslot error!\n");
        return NULL;
    }
    if(buffer->slots[slot->next].refs > 0) 
    {
        /*下一个slot非空，必须增加一页slot*/
        sprintf(szDebug,"######must add a new page of slot now, buffer adder is %x",buffer);
        ...
        if ( !(slot=OMSbuff_addpage(buffer, slot)) ) 
        {
            ...
            sprintf(szDebug,"!!!!!!omsbuff add page of slot error !!!");
            DEBUG_PRINTF(szDebug);
    ...
    /*DEBUG_PRINTF("________call omsbuff get slot ok!\n");*/   /*just for debug,yanf*/
    return slot; 
}	
