/* * 
 *  $Id: stream_change.c 32 2004-07-07 09:50:50Z shawill $
 *  
...
int stream_change(RTP_session *changing_session, int value)
{
    if (value == 0) 
    {
        ...
        {
            /*如果没有达到最大连接，将该标志位重置为0，执行具体操作，下同*/
            if (changing_session->MinimumReached != 1) 
            {
                changing_session->MaximumReached = 0;
                if (strcmp(changing_session->current_media->description.encoding_name,"MPA")==0)
                    return downgrade_MP3(changing_session);
                if (strcmp(changing_session->current_media->description.encoding_name,"GSM")==0)
                    return downgrade_GSM(changing_session);
                if (strcmp(changing_session->current_media->description.encoding_name,"L16")==0) 
